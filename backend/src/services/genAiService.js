...
async function answerFarmerQuery({
  ...
  const baseSystemPrompt = buildSystemPromptEnvelope(diagnosisContext, weatherContext, targetLang);
  ...
  if (isKeyConfigured) {
    try {
      ...
      console.warn('Gemini API request failed or timed out. Falling back to deterministic grounded response:', err.message);
    }
  }
